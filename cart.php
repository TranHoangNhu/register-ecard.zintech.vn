...
if(isset($_GET['mahangban']))			//-----lay ma hang ban bang phuong thuc get
{
	...
	$rs7=$conn->query($l_sql)->fetchAll(PDO::FETCH_ASSOC);
	if($rs7)
	{
		foreach($rs7 as $r7)
		{
...
//-----lay ma hang ban bang phuong thuc post
...
<?php
	...
	if ($mahangban != "" && $action!="update" and $action!="remove")//-- dang chon mon ok
	{
		if (!array_key_exists($mahangban,$_SESSION['TenHangBan']))//---kiểm tra có key chưa ?
		{
			...
			$rs3=$conn->query($l_sql)->fetchAll(PDO::FETCH_ASSOC);
			if($rs3)
			{
				foreach($rs3 as $r3)
				{
	...
	if ($action=="update") 
	{
		...
		for ($i=0;$i<count($_POST['mahangban_arr']);$i++) 
		{
			...
			$rs4=$conn->query($l_sql_update)->fetchAll(PDO::FETCH_ASSOC);
			if($rs4)
			{
				foreach($rs4 as $r4)
				{
