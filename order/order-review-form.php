...
<?php
	...
	if ($maktv != "" && $action!="update" and $action!="remove")//-- dang chon mon ok
	{
			...
			$rs3=$conn->query($l_sql)->fetch();
			
  		    $_SESSION['TenKTV'][$maktv]=$rs3['TenNV']; var_dump($_SESSION['MaKTV']);
			$_SESSION['MaKTV'][$maktv]=$rs3['MaNV'];
	...
	if ($action=="update") 
	{
		...
		for ($i=0;$i<count($_POST['maktv_arr']);$i++) 
		{
			...
			$rs3=$conn->query($l_sql_update)->fetch();
				
			$_SESSION['TenKTV'][$maktv]=$rs4['TenNV'];
			$_SESSION['MaKTV'][$maktv]=$rs4['MaNV'];
