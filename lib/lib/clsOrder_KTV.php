...
class clsOrder_KTV {
	...
	public function getTenKTV( $maktv ){
		...
		try 
		{
			$rs = $this->conn->query($sql)->fetch();//All(PDO::FETCH_ASSOC);//sqlsrv_query($this->conn, $sql, array(), array( "Scrollable" => 'static' ));
	...
	public function updateKTVtoOrderID($ma_KTV, $ten_KTV, $maban, $malichsuphieu  ) {
			...
			try
			{
				$rs = $this->conn->query($sql);//->fetchAll(PDO::FETCH_ASSOC);//sqlsrv_query($this->conn, $sql);
			}
			catch ( PDOException $error ){
				echo $error->getMessage();
			}
	   ...
	   var_dump ( $_FILES['files']['name'] );
		...
		if(!empty($fileNames))
		{
			foreach( $fileNames as $key => $value )
			{
		 	...
		 	{
		 		$sql = "UPDATE [tblDMNhanVien] SET SourceHinhAnh = '$img_files' WHERE MaNV = '$maktv'";
		 		$rs = $this->conn->query($sql);//->fetchAll(PDO::FETCH_ASSOC);//sqlsrv_query($this->conn, $sql);
	...
	public function getKTVPicsByID( $maktv ){
		...
		try
			{
				$rs = $this->conn->query($sql)->fetch();//All(PDO::FETCH_ASSOC);//sqlsrv_query($this->conn, $sql);
				//$r = sqlsrv_fetch_array( $rs );
	...
	public function getHangBan(){
		...
		$rs = $this->conn->query($sql)->fetchAll(PDO::FETCH_ASSOC);//sqlsrv_query($this->conn, $sql)->fetchAll( \PDO::FETCH_ASSOC );
		return $rs;
	}
}